...
def tokenize_function_qa(examples):
    """
    Tokenize the given examples containing questions and answers using a pre-defined tokenizer.

    Args:
        examples (pd.DataFrame): A DataFrame containing two columns: "bold_questions" with text samples
                                 representing questions and "answer" with text samples representing answers.

    Returns:
        transformers.tokenization_utils_base.BatchEncoding: A batch encoding object containing the tokenized
        input sequences (questions and answers), with truncation and padding applied.
    """
    return tokenizer(
        examples["bold_questions"].tolist(),
        examples["answer"].tolist(),
        truncation=True,
        padding=True,
    )
...
train_encodings = tokenize_function_qa(train_data)
val_encodings = tokenize_function_qa(val_data)
test_encodings = tokenize_function_qa(test_data)
...
training_args = TrainingArguments(
    output_dir="./RoBERTa_QA",  # output directory
...
trainer.train()
trainer.evaluate()
trainer.save_model("RoBERTa_QA")
tokenizer.save_pretrained("RoBERTa_QA")
trainer.save_metrics("RoBERTa_QA", metrics=trainer.evaluate())
y_true = test_labels
