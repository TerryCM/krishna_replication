...
def tokenize_function_q(examples):
    """
    Tokenize the given examples using a pre-defined tokenizer.

    Args:
        examples (pd.DataFrame): A DataFrame containing a column "bold_questions" with text samples to tokenize.

    Returns:
        transformers.tokenization_utils_base.BatchEncoding: A batch encoding object containing the tokenized
        input sequences, with truncation and padding applied.
    """
    return tokenizer(examples["bold_questions"].tolist(), truncation=True, padding=True)
